...
func TestRender_JsonTagWithStringOption_ForcesStringType(t *testing.T) {
	type VoteOptionDetailResponse struct {
		// Id 选项ID
		Id int64 `json:"id,string"`
	}

	go2ts := New()
	go2ts.Add(VoteOptionDetailResponse{})
	var b bytes.Buffer
	err := go2ts.Render(&b)
	require.NoError(t, err)
	expected := `// DO NOT EDIT. This file is automatically generated.

export interface VoteOptionDetailResponse {
	id: string;
}
`
	assert.Equal(t, expected, b.String())
...
func TestRender_JsonTagWithStringOption_MultipleTypes(t *testing.T) {
	type Response struct {
		Id          int64   `json:"id,string"`
		UserId      int     `json:"user_id,string"`
		Count       int32   `json:"count,string"`
		Price       float64 `json:"price,string"`
		NormalInt   int64   `json:"normal_int"`
		NormalFloat float64 `json:"normal_float"`
...
func TestRender_WithComments_PreservesComments(t *testing.T) {
	type VoteOptionDetailResponse struct {
		// Id 选项ID
		Id int64 `json:"id,string"`
	}
...
// VoteOptionDetailResponse 投票选项详情响应
type VoteOptionDetailResponse struct {
	// Id 选项ID
	Id int64 ` + "`json:\"id,string\"`" + `
}`
	err := go2ts.ParseCommentsFromSource(source)
	require.NoError(t, err)
